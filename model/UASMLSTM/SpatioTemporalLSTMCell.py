...
class SMLSTMCell(nn.Module):
    def __init__(self, num_hidden_in, num_hidden, width, higth, filter_size,layer_norm, forget_bias=1.0, device='cuda'):
        """SM-LSTM 单元
        Args:
            num_hidden_in: 输入通道数
            num_hidden: 隐藏单元通道数
        ...
        if layer_norm:
            self.conv_x = nn.Sequential(
                DepthwiseSeparableConv(num_hidden_in, num_hidden * 7, kernel_size=filter_size, stride=1, padding=self.padding),
                nn.LayerNorm([num_hidden * 7, self.height, self.width])
            ).to(device)
            self.conv_h = nn.Sequential(
                DepthwiseSeparableConv(num_hidden, num_hidden * 4, kernel_size=filter_size, stride=1, padding=self.padding),
                nn.LayerNorm([num_hidden * 4, self.height, self.width])
            ).to(device)
            self.conv_c = nn.Sequential(
                DepthwiseSeparableConv(num_hidden, num_hidden * 3, kernel_size=filter_size, stride=1, padding=self.padding),
            ...
            self.conv_x = DepthwiseSeparableConv(num_hidden_in, num_hidden * 7, kernel_size=filter_size, stride=1, padding=self.padding).to(device)
            self.conv_h = DepthwiseSeparableConv(num_hidden, num_hidden * 4, kernel_size=filter_size, stride=1, padding=self.padding).to(device)
            self.conv_c = DepthwiseSeparableConv(num_hidden, num_hidden * 3, kernel_size=filter_size, stride=1, padding=self.padding).to(device)
            self.conv_m = DepthwiseSeparableConv(num_hidden, num_hidden * 3, kernel_size=filter_size, stride=1, padding=self.padding).to(device)
            self.conv_c2m = DepthwiseSeparableConv(num_hidden, num_hidden * 4, kernel_size=filter_size, stride=1, padding=self.padding).to(device)
            self.conv_m2o = DepthwiseSeparableConv(num_hidden, num_hidden, kernel_size=filter_size, stride=1, padding=self.padding).to(device)

        self.conv_last = nn.Conv2d(num_hidden * 2, num_hidden, kernel_size=1, stride=1, padding=0, bias=False).to(device)
    ...
    def forward(self, x_t, h_t, c_t, m_t):
        ...
        c_cc = self.conv_c(h_t)
        m_cc = self.conv_m(h_t)
